...
class OrderManager:
    ...
    def place_orders(self, signal: Signal, account_config: Dict[str, Any]) -> List[int]:
        ...
        # Get break-even trigger price for comment
        # For ICM signals: use TP2 (second non-"open" TP)
    ...
    def _apply_breakeven_to_positions(self, positions: List[Any]):
        ...
        for position in positions:
            try:
                # For manual breakeven command, always move SL to entry price (no offset)
                ...
                if success:
                    logger.info(f"✅ Breakeven applied to position {position.ticket} - SL moved to {new_sl} (entry price)")
                else:
